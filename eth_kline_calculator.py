...
- 动量: {patterns['price_action']['momentum']}
    ...
    def predict_future_kline(self, days=5):
        """
        预测未来N天的K线走势
        基于历史数据和技术指标进行预测
        """
        if not self.kline_data or len(self.kline_data) < 10:
            return []
        
        # 获取历史数据用于预测
        historical_data = self.kline_data[-20:]  # 使用最近20天数据
        predictions = []
        ...
        latest_volume = historical_data[-1]['volume']
        
        # 计算价格和交易量的移动平均
        recent_prices = [day['close'] for day in historical_data[-10:]]
        recent_volumes = [day['volume'] for day in historical_data[-10:]]
        ...
        volume_trend = volume_patterns.get('volume_trend', 0) / 100
        ...
        for i in range(days):
            ...
            if trend_direction == 'uptrend':
                base_price_change = trend_strength * (i + 1) * 0.02
            elif trend_direction == 'downtrend':
                base_price_change = -trend_strength * (i + 1) * 0.02
            else:
                base_price_change = random.uniform(-0.01, 0.01) * (i + 1)
            
            # 添加随机波动 - 更小的波动范围
            random_factor = np.random.normal(0, price_std * 0.1)
            
            # 计算预测价格
            predicted_close = latest_price * (1 + base_price_change + random_factor)
            
            # 确保价格合理性 - 更保守的波动范围
            predicted_close = max(predicted_close, latest_price * 0.85)  # 最低不低于当前价格85%
            predicted_close = min(predicted_close, latest_price * 1.15)  # 最高不超过当前价格115%
            
            # 计算日内波动
            daily_volatility = price_std * random.uniform(0.5, 1.5)
            
            # 计算开盘价（基于前一交易日收盘价）
            if i == 0:
                predicted_open = latest_price
            else:
                predicted_open = predictions[i-1]['close']
            
            # 计算最高价和最低价
            predicted_high = predicted_close + random.uniform(0, daily_volatility)
            predicted_low = predicted_close - random.uniform(0, daily_volatility)
            ...
            # 预测交易量
            volume_change = volume_trend * (i + 1) * 0.1
            volume_random = np.random.normal(0, volume_std * 0.2)
            predicted_volume = latest_volume * (1 + volume_change + volume_random)
            predicted_volume = max(predicted_volume, latest_volume * 0.3)
            
            # 计算技术指标预测
            predicted_technical = self.predict_technical_indicators(historical_data + predictions, {
                'open': predicted_open,
                'high': predicted_high,
                'low': predicted_low,
            ...
            # 计算市场情绪因子
            market_sentiment = self.calculate_market_sentiment(len(historical_data) + i)
            ...
            prediction = {
                ...
                'volume_usd': round(predicted_volume * 1000000000, 0),
                'change': round(predicted_close - predicted_open, 2),
                'change_percent': round((predicted_close - predicted_open) / predicted_open * 100, 2),
                'technical': predicted_technical,
                'market_data': {
                    'dominance': 18.5 + random.uniform(-1, 1),
                    'market_cap_rank': 2,
                    'exchanges_count': 4,
                    'sentiment_score': round(market_sentiment, 2)
                },
                'confidence': self.calculate_prediction_confidence(i, trend_strength, volume_std)
            }
    ...
    def predict_technical_indicators(self, historical_data, current_price_data):
        """
        预测技术指标
        """
        if len(historical_data) < 2:
    ...
    def calculate_prediction_confidence(self, day_offset, trend_strength, volume_std):
        """
        计算预测置信度
        置信度随预测天数递减
        """
        ...
        # 根据预测天数递减
        time_decay = 0.95 ** day_offset
        
        # 根据趋势强度调整
        trend_factor = 1 + trend_strength * 0.1
        
        # 根据历史波动率调整
        volatility_factor = 1 - min(volume_std / 100, 0.2)
        
        confidence = base_confidence * time_decay * trend_factor * volatility_factor
    ...
    def analyze_prediction_patterns(self, predictions):
        ...
        # 计算预测趋势
        first_price = predictions[0]['close']
        last_price = predictions[-1]['close']
        total_change = (last_price - first_price) / first_price
        
        # 分析涨跌天数
        up_days = sum(1 for day in predictions if day['close'] > day['open'])
        down_days = len(predictions) - up_days
        
        # 计算平均波动率
        volatilities = []
        for day in predictions:
            daily_range = (day['high'] - day['low']) / day['open']
            volatilities.append(daily_range)
        avg_volatility = np.mean(volatilities)
        
        # 计算价格目标
        price_targets = {
            'conservative': first_price * (1 + total_change * 0.5),
            'moderate': first_price * (1 + total_change),
            'optimistic': first_price * (1 + total_change * 1.5)
        }
        ...
        return {
            ...
            'up_days': up_days,
            'down_days': down_days,
            'bullish_ratio': round(up_days / len(predictions), 2),
            'avg_daily_volatility': round(avg_volatility * 100, 2),
            'price_targets': {k: round(v, 2) for k, v in price_targets.items()},
            'trend_direction': 'bullish' if total_change > 0.02 else 'bearish' if total_change < -0.02 else 'neutral'
        }
...
if __name__ == "__main__":
    ...
    for i in range(15):
        ...
        test_volume_data.append({
            'date': date,
            'total_volume': 15.2 + random.uniform(-3, 3),
            'exchanges': {
                'binance': {'volume': 5.3 + random.uniform(-1, 1)},
                'coinbase': {'volume': 3.8 + random.uniform(-0.5, 0.5)},
                'kraken': {'volume': 2.1 + random.uniform(-0.3, 0.3)},
                'okx': {'volume': 3.0 + random.uniform(-0.5, 0.5)}
            }
        })
    ...
    # 测试预测功能
    print("\n" + "="*50)
    print("🔮 预测未来5天ETH K线走势")
    print("="*50)
    
    predictions = calculator.predict_future_kline(5)
    if predictions:
        print(f"\n📊 预测结果 (基于最近{len(calculator.kline_data)}天数据):")
        
        for i, pred in enumerate(predictions, 1):
            print(f"\n第{i}天 ({pred['date']}):")
            print(f"  开盘: ${pred['open']:,.2f}")
            print(f"  最高: ${pred['high']:,.2f}")
            print(f"  最低: ${pred['low']:,.2f}")
            print(f"  收盘: ${pred['close']:,.2f}")
            print(f"  涨跌: {pred['change']:+.2f} ({pred['change_percent']:+.2f}%)")
            print(f"  交易量: {pred['volume']:.2f} 十亿美元")
            print(f"  置信度: {pred['confidence']:.1%}")
        
        # 分析预测模式
        patterns = calculator.analyze_prediction_patterns(predictions)
        print(f"\n📈 预测分析:")
        print(f"  总体趋势: {patterns['trend_direction']}")
        print(f"  总涨跌幅: {patterns['total_change_percent']:+.2f}%")
        print(f"  上涨天数: {patterns['up_days']}/{len(predictions)}")
        print(f"  多头比例: {patterns['bullish_ratio']:.1%}")
        print(f"  平均日波动率: {patterns['avg_daily_volatility']:.2f}%")
        print(f"  价格目标:")
        for target_type, price in patterns['price_targets'].items():
            print(f"    {target_type}: ${price:,.2f}")
    else:
        print("❌ 预测失败：历史数据不足")
